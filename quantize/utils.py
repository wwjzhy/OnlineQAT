...
def set_weight_parameters(model, requires_grad):
    ...
    for n, m in model.named_parameters():
        if n.find('weight') > -1 and not (n.find('scale') > -1 or n.find('zero_point') > -1 or n.find('output_scale') > -1):
...
def weight_parameters(model):
    params = []
    for n, m in model.named_parameters():
        if n.find('weight') > -1 and not(n.find('scale') > -1 or n.find('zero_point') > -1 or n.find('norm') > -1 or n.find('output_scale') > -1):
            # print(f"add weight_parameters: {n}")
            params.append(m)
    return iter(params)
...
def set_quant_parameters(model, requires_grad):
    params = []
    for n, m in model.named_parameters():
        if n.find('scale') > -1 or n.find('zero_point') > -1 or n.find('output_scale') > -1:
            m.requires_grad = requires_grad
    return iter(params)  

def set_layernorm_parameters(model, requires_grad):
    params = []
...
def quant_parameters(model):
    params = []
    for n, m in model.named_parameters():
        if n.find('scale') > -1 or n.find('zero_point') > -1 or n.find('norm') > -1 or n.find('output_scale') > -1:
            # print(f"add quant_parameters: {n}")
            params.append(m)
    return iter(params)  
...
def quant_inplace(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, QuantLinear):
            ...
            # delete scale and zero_point parameters to avoid key error
